...
function generateIdForIdeas() {
  return (ideaIdCounter++).toString();
}

function generateIdForMinions() {
  const allMinions = getAllFromDatabase('minions'); // get current minions
  const ids = allMinions.map(m => Number(m.id)).sort((a, b) => a - b);

  // Find the first missing number starting from 1
  for (let i = 1; i <= ids.length + 1; i++) {
    if (!ids.includes(i)) return i.toString(); // return as string
  }

  return (ids.length + 1).toString(); // fallback
}
...
const addToDatabase = (modelType, instance) => {
 ...
 if (model.isValid(instance)) {
  if (modelType === 'minions') {
    instance.id = generateIdForMinions();
  } else if (modelType === 'ideas') {
    instance.id = generateIdForIdeas();
  }
  // ... same for meetings or work
...
function addWorkToDatabase(minionId, workData) {
  // Find the minion
  const minion = getFromDatabaseById('minions', minionId);
  if (!minion) {
    return null; // minion not found
  }

  // Create the new work object
  const newWork = { ...workData, id: generateId(), minionId };

  // Ensure minion has a work array
  if (!Array.isArray(minion.work)) {
    minion.work = [];
  }

  // Push the new work into the minion's work array
  minion.work.push(newWork);
...
const updateInstanceInDatabase = (modelType, instance) => {
  ...
  const index = model.data.findIndex(el => el.id.toString() === instance.id.toString());
  if (index === -1) return null;

  try {
    if (!model.isValid(instance)) return null; // validation fails → return null
  } catch (err) {
    return null; // any thrown error → return null
  }
...
const deleteFromDatabasebyId = (modelType, id) => {
  ...
  const index = model.data.findIndex(el => el.id.toString() === id.toString());
  if (index !== -1) {
    // remove the item and return it
    return model.data.splice(index, 1)[0];
  }
...
//   const index = model.data.findIndex(el => el.id.toString() === id.toString());
//   if (index !== -1) {
//     const deleted = model.data.splice(index, 1)[0];
//     return deleted;
//   }
//   return null;
// };
